...
(function () {
  ...
  function tabTexts(body) {
    const T = draft.texts;
    const single = [["omenTag", "地鳴標籤"], ["omenEnter", "地鳴開始"], ["chanceLose", "地鳴平息"], ["chanceCollapse", "演出失敗：坍塌"], ["chanceWin", "演出成功"], ["revive", "復活"], ["directWin", "金礦直擊"], ["stock", "礦脈延伸告知"], ["atHighTag", "共鳴標籤"], ["atHighTagDeep", "深層共鳴標籤"], ["atHighEnterDeep", "深層共鳴開始"], ["atHighEnter", "共鳴開始"], ["atHighHint", "共鳴中暗示"], ["atHighEnd", "共鳴結束"], ["upgrade", "礦脈變粗告知"], ["bonusChain", "礦脈延伸揭曉"], ["bonusChainSurprise", "最後一揮才揭曉"], ["bonusEnd", "礦脈結束"], ["veinCap", "礦脈達上限"], ["upperStart", "進入核心層(上位)"], ["koukakuHint", "高確暗示"], ["hintSfx", "違和感：音效"], ["hintDrip", "違和感：水滴"], ["hintGlow", "違和感：紋路"], ["hintTap", "違和感：點擊提示"], ["toolDrop", "工具掉落"], ["toolBreak", "工具損壞"], ["noTool", "沒有工具"], ["tap", "點擊提示"]];
    body.innerHTML = `
  ...
  const LABEL = {
    ...
    first: "一般當選", fromEpic: "發展中靠紫", fromLegend: "靠金礦", next: "連莊下一隻", upper: "上位（第10隻後）", upperAfter: "第幾隻起進上位",
    length: "長度(揮)", cap: "一次礦脈總揮數上限", swings: "揮數", cont: "延伸（連莊）抽選", atHigh: "礦層共鳴（AT中的高確）", boost: "第5隻以後（深層共鳴）", enterEpic: "紫礦→進入共鳴", enterLegend: "金礦→進入共鳴", contEpic: "共鳴中紫礦→延伸", contLegend: "共鳴中金礦→延伸", boostAfter: "第幾隻起加強", low: "第1～4隻", high: "加強後", add: "挖到稀有礦加成", upgrade: "升格率", RBtoBB: "RB→BB", BBtoSBB: "BB→SBB", announceRate: "當下告知機率",
    ...
    upAt: "從第幾色用升溫台詞", hotAt: "從第幾色用確信台詞", hit: "ST對應牌通過率", call: "找你談話", guarantee: "保底（第幾個必定）", rate: "各累積數的機率",
    scene: "劇本抽選（外出／激熱）", strong: "外出", colorTarget: "最後會升到哪一色的權重", colorCeil: "各劇本的顏色上限", colorFloor: "各劇本的起始色", colorStartAt: "演到幾成才開始升色",
    bigIncs: "鏟子・大跳的數字", bigAfter: "前幾下不出現大跳", maxTaps: "鏟子最多點幾下", perTap: "每幾轉配一下點擊",
  ...
  function tabM2(body) {
    ...
    body.innerHTML = `
      ...
      ${M.bosses.map((b, i) => `
        ...
        ${["strong", "hot"].map(sc => `
          <div class="ed-sub">${sc === "strong" ? "外出" : "激熱"}　<input data-p="machine2.bossScenes.${b.id}.${sc}.place" value="${esc((((M.bossScenes || {})[b.id] || {})[sc] || {}).place || "")}" style="width:120px"></div>
          ${SKEYS.map(([k, n]) => `<div class="ed-row"><label>${n}</label><textarea rows="2" data-p="machine2.bossScenes.${b.id}.${sc}.${k}" data-lines>${esc(((((M.bossScenes || {})[b.id] || {})[sc] || {})[k] || []).join("\n"))}</textarea></div>`).join("")}
        `).join("")}
      `).join("")}
      ...
      <div class="ed-note" style="margin:6px 0 2px">好感度 +10%（＝被找去談話的機率）</div>
      <div class="ed-flex">${M.bosses.map(b => `<button class="ed-btn" data-m2cnt="${b.id}">${b.name} +10%</button>`).join("")}</div>
    ...
    dev("[data-m2cnt]", b => { const id = b.dataset.m2cnt; G.m2.setFavor(id, (G.m2.state.favor[id] || 0) + 0.1); G.toast("好感度 +10%"); });
